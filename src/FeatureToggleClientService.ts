...
export class FeatureToggleClientService {
    ...
    private constructor() {
        if (FeatureToggleClientService.instance) {
            throw new Error('FeatureToggleClientService is a singleton. Use getInstance() method instead of constructor');
        }
    }

    static getInstance(): FeatureToggleClientService {
        FeatureToggleClientService.instance = FeatureToggleClientService.instance || new FeatureToggleClientService();
    ...
    initializeApplication(payload: Application): void {
        this.applicationInstance = new FeatureToggleInstanceFactory(payload).getClient();
    }
     ...
     * @param defaultValue
     */
    isUserFeatureEnabled(featureKey: string, defaultValue: boolean = false): Promise<{}> {
     ...
     * @param defaultValue
     */
    isApplicationFeatureEnabled(featureKey: string, defaultValue: boolean = false): Promise<{}> {
        const withApplicationFeaturePromise = new Promise(async (resolve, reject) => {
            try {
                await this.applicationInstance.waitUntilReady();
                resolve(this.applicationInstance.variation(featureKey, defaultValue));
            } catch (e) {
                reject(`Error while initializing LDClient (application instance): ${e}`);
            }
        });
